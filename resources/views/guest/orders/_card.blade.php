<a class="guest-order-card" data-order-id="{{ $order->public_id }}" data-order-status="{{ $order->status->value }}" data-payment-status="{{ $order->payment_status }}" href="{{ route('guest.orders.show', [$company, $order]) }}">
    <span class="order-status-icon status-{{ $order->status->value }}"><i class="bi {{ $order->hasRefund() ? 'bi-arrow-counterclockwise' : (in_array($order->status, [\App\Enums\RequestStatus::Ready, \App\Enums\RequestStatus::Completed], true) ? 'bi-check-lg' : ($order->status === \App\Enums\RequestStatus::Cancelled ? 'bi-x-lg' : 'bi-hourglass-split')) }}"></i></span>
    <div class="order-card-copy">
        ...
        <h2>{{ $order->items->first()?->name_snapshot ?? $order->title }}</h2>
