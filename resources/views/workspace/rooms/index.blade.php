...
<div class="surface-card form-section mb-4">
    ...
    <form class="row g-3 align-items-end" method="POST" action="{{ route('workspace.rooms.store') }}">@csrf
        ...
        <div class="col-md-5"><label class="form-label">{{ __('workspace.room_name') }}</label><input class="form-control" name="name" maxlength="160" value="{{ old('name') }}" placeholder="{{ __('workspace.room_name_placeholder') }}"><div class="form-hint mt-1">{{ __('workspace.room_name_hint') }}</div></div>
...
@forelse($rooms as $room)
    <form class="surface-card room-inventory-row" method="POST" action="{{ route('workspace.rooms.update', $room) }}">@csrf @method('PUT')
        ...
        <div><label class="form-label">{{ __('workspace.room_name') }}</label><input class="form-control" name="name" maxlength="160" value="{{ $room->name }}" placeholder="{{ __('workspace.room_name_placeholder') }}"><div class="form-hint mt-1">{{ __('workspace.room_name_hint') }}</div></div>
