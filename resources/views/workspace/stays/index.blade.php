...
<tr>
    ...
    <td><span class="room-chip">{{ $stay->room->number }}</span></td>
    ...
    <td class="small fw-semibold">{{ $money->format((int)$stay->requests->where('payment_method','room_charge')->where('status','!=',\App\Enums\RequestStatus::Cancelled)->sum('price_minor'),$currentCompany->currency) }}</td>
...
<div class="modal fade" id="newStayModal" tabindex="-1"><div class="modal-dialog modal-lg modal-dialog-centered"><form class="modal-content" method="POST" action="{{ route('workspace.stays.store') }}">@csrf<div class="modal-header"><div><h2 class="modal-title fs-5">{{ __('workspace.new_stay') }}</h2><div class="text-secondary small">{{ __('workspace.new_stay_hint') }}</div></div><button class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-4"><div class="row g-3">
    ...
    <div class="col-md-5"><label class="form-label">{{ __('workspace.room') }} *</label><select class="form-select" name="room_id" required><option value="">{{ __('workspace.choose_room') }}</option>@foreach($rooms as $room)<option value="{{ $room->id }}" @selected(old('room_id')==$room->id)>{{ $room->number }}</option>@endforeach</select></div>
