...
<main class="print-bill-sheet">
    ...
    <table class="print-bill-table"><thead><tr><th>Service</th><th>Date</th><th>Payment</th><th>Amount</th></tr></thead><tbody>
        @forelse($orders as $order)<tr><td><strong>{{ $order->items->first()?->name_snapshot ?? $order->title }}</strong><small>Order #{{ str($order->public_id)->substr(0, 8)->upper() }}</small></td><td>{{ $order->created_at->setTimezone($currentCompany->timezone)->format('M j, Y') }}</td><td>{{ $order->payment_status === 'paid' ? 'Paid' : 'Amount due' }}</td><td>{{ $money->format($order->price_minor, $currentCompany->currency) }}</td></tr>
