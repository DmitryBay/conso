...
@forelse($logs as $log)
<tr><td class="small text-nowrap">{{ $log->created_at->setTimezone($currentCompany->timezone)->format('d.m.Y H:i:s') }}</td><td><strong class="small">{{ $log->manager?->name ?? '—' }}</strong><div class="text-secondary" style="font-size:10px">{{ $log->ip_address }}</div></td><td><span class="badge-soft-secondary">{{ __('workspace.action_log_actions.'.str_replace('.','_',$log->action)) }}</span></td><td>@if($log->serviceRequest)<a class="small fw-semibold" href="{{ route('workspace.requests.show',$log->serviceRequest) }}">{{ $log->serviceRequest->title }}</a><div class="text-secondary" style="font-size:10px">#{{ str($log->serviceRequest->public_id)->substr(0,8)->upper() }}</div>@else<span class="text-secondary">—</span>@endif</td><td class="small">@forelse($log->metadata ?? [] as $key=>$value)<span class="audit-meta"><b>{{ __('workspace.action_log_fields.'.$key) }}:</b> {{ is_bool($value) ? ($value ? __('workspace.yes') : __('workspace.no')) : $value }}</span>@empty<span class="text-secondary">—</span>@endforelse</td></tr>
